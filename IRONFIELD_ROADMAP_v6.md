# IRONFIELD — Development Roadmap v6
*Updated: 2026-04-07*
...
- Single HTML file (`index.html`), ~4365 lines, no build step
...
### ✅ Completed — 2.4 Elevation
See full notes in the completed section above. Terrain system is now fully complete.
...
## TIER 7 — Physical combat

Physical attacks become relevant under the WeGo system because mechs can end up in the same hex or adjacent hexes after simultaneous movement — and the system needs rules for what happens when that occurs.

### 7.1 Collision / Charging detection
When both players submit movement plans and `resolveTurn` applies moves, check for:
- **Same-hex collision**: two mechs (one from each player) end their move in the same hex. Both take damage. The heavier mech wins the push and stays; the lighter is displaced to the nearest empty adjacent hex.
- **Head-on charge**: a mech ends its move adjacent to an enemy that was moving *toward* it. Speed differential drives damage — the faster the combined approach, the harder the hit.

**Detection approach:** After applying all moves in `resolveTurn`, walk all mech pairs and test hex proximity. A collision is any same-player-vs-enemy pair within 1 hex where neither mech started there (both moved into proximity this turn).

**Damage model:**
- Charge damage = `floor(mpSpent / 3)` per mech, applied to a random facing location (front arc preferred). Both mechs take damage — attacker takes half, defender takes full.
- Same-hex displacement: treat as charge plus forced facing change.

### 7.2 Deliberate physical attack order
Allow a player to queue a **CHARGE** order in the planning phase: select own mech, select enemy mech as target, commit all remaining MP to movement toward that target. On resolution, if the charging mech ends adjacent to or in the target hex, the physical attack resolves automatically. If the charge falls short (enemy moved away), the mech just ends in the closest reachable hex.

**UI:** A `[ CHARGE ]` button in the fire panel, enabled when a mech and enemy are both selected. Queuing a charge locks the mech's movement destination and prevents other fire orders from that mech (you can't shoot and charge in the same turn).

### 7.3 Push / shove (melee attack)
A shorter-range physical action: mech spends 2 MP to push an adjacent enemy. The pushed mech is displaced one hex in the direction of the shove. No damage, but forces enemy repositioning that can break cover or push into terrain. Relevant once more chassis types exist (assault-class mechs are natural pushers).

### 7.4 Fallen mech state
After a heavy charge or leg destruction, a mech may fall:
- `mech.fallen = true` — mech is prone, cannot move or fire until it stands
- Standing costs 3 MP and a piloting roll (3D6 vs TN 8 + piloting modifier)
- Fallen mechs are easier to hit: -2 TN on all shots against them

### Notes for implementation
- 7.1 (collision detection) is the highest priority and can be added to `resolveTurn` as a post-movement pass with no new UI
- 7.2 (charge order) requires UI work but reuses the existing fire order architecture
- 7.3 and 7.4 are polish items; defer until 7.1/7.2 are playtested
...
1. **VS AI — Basic difficulty (4.1/4.2)** — Greedy AI enables solo testing without a second player. Single session. Highest practical value if multiplayer setup is friction.
2. **Physical combat — Collision detection (7.1)** — Adds WeGo-specific tension. Purely a `resolveTurn` addition with no new UI. Can be done in half a session alongside something else.
3. **Additional chassis + weapons (3.1/3.2)** — More variety in the lance picker. Can be done incrementally.
4. **VS AI — Hard difficulty with Claude API** — Prompt board state JSON, receive plan object.
5. **Campaign mode (Tier 5)** — Largest remaining scope; multi-session.
6. **Quality of life (Tier 6)** — Replay system, turn timer, sound — polish items; prioritise after core is proven fun.
